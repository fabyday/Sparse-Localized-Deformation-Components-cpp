...
void SplocsSolver::get_local_support(const int idx, MatrixXR& result, real_type min, real_type max)
{
	...
	//result = (D.array() > min) = min; .min(max).max(min));
...
void SplocsSolver::scale_meshes() {
	...
	MatrixXR mean_V;
	mean_V.resizeLike(ref.V);
	mean_V.setZero();
	real_type v_scale = 0;
	for (int i = 0; i < meshes_.size(); i++) {
		Mesh& m = meshes_[i];
		mean_V += m.V / meshes_.size();
...
void SplocsSolver::find_rbm_procrustes()
{
	...
	for (int i = 0; i < meshes_.size(); i++) {
		...
		m.V = std::move(( T0* m_tmp.transpose()).transpose().block(0, 0, rows, 3));
